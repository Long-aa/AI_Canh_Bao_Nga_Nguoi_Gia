      ...
      <Layout>
        <div className="max-w-[1600px] mx-auto space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Chào buổi sáng, Quản trị viên</h1>
              <div className="flex items-center gap-2 mt-1.5 text-slate-500">
                ...
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-green-100 text-green-700 ml-2 animate-pulse">
                  REAL-TIME
                </span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-all font-semibold text-sm">
                Tải báo cáo
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => {
              ...
              return (
                <div key={index} className="group bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Phân tích tần suất cảnh báo</h2>
                  <p className="text-slate-500 text-sm mt-1">Dữ liệu tổng hợp trong 7 ngày gần nhất</p>
                </div>
                <select className="bg-slate-50 border-none rounded-lg text-sm font-bold text-slate-600 focus:ring-2 focus:ring-blue-500/20">
            ...
            <div className="bg-slate-900 rounded-3xl shadow-xl p-8 text-white relative overflow-hidden">
              ...
              <div className="space-y-6">
                ...
                <div className="mt-10 p-6 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl shadow-lg">
                  ...
                  <h3 className="text-xl font-bold mt-1">Cần hỗ trợ?</h3>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900">Thông báo khẩn cấp mới nhất</h2>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50/50">
                    <th className="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">Thời gian</th>
                    <th className="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">Người dùng</th>
                    <th className="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">Vị trí</th>
                    <th className="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">Mức độ</th>
                    <th className="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">Trạng thái</th>
                    <th className="px-8 py-4 text-right text-xs font-bold text-slate-400 uppercase tracking-widest">Thao tác</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {recentAlerts.map((alert, index) => (
                    <tr key={index} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="text-slate-900 font-bold">{alert.person}</div>
                      </td>
                      <td className="px-8 py-5 whitespace-nowrap">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${alert.status === 'Đang xử lý' ? 'bg-red-500 animate-ping' : 'bg-slate-300'}`}></div>
                          <span className="text-sm font-bold text-slate-700">{alert.status}</span>
                        </div>
                      </td>
                      <td className="px-8 py-5 whitespace-nowrap text-right">
                        <button className="bg-slate-100 group-hover:bg-blue-600 group-hover:text-white text-slate-700 p-2 rounded-lg transition-all">
